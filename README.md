...
│       ├── check-lab-connectivity.sh   # fail-fast TCP check to Catalyst Center
│       ├── prepare-job.sh
...
| Commit reports fails with `! [rejected] ... non-fast-forward` | Run executed at a trigger SHA that trails master (typical when re-running an older run) | Fixed — `commit-reports.sh` fetches and moves to `origin/master` before committing the run directory. |
| Activation fails: `Failed to activate <image> to <devices>` | Target image matches the version already running on the devices, so the CatC activation task ends in FAILURE with `errorCode 100` | Compare `upgrade_image` against the running version (`network_device_info` → `softwareVersion`). This fails the same way in 01.3 and 02.1, regardless of `activate_lower_image_version`. |
| 01.3 reports success but nothing changed: `Devices not eligible for activation` | Image never reached the devices (distribution skipped/failed) or CatC considers them ineligible | The module returns `status: success` here, so check the 01.3 evidence JSON `response` text, not just the play recap |
