...
| Activation fails: `Failed to activate <image> to <devices>` | `upgrade_image` is *older* than the running image; stage 01.3 sets `activate_lower_image_version: false` | Downgrades must run through `02.1_swim_rollback.yml` (it tags `rollback_image` with `activate_lower_image_version: true`). Setting `upgrade_image` to an older release will not downgrade devices. |
